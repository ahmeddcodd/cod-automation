...
async def wait_and_cancel(ctx, step):
    ...
    # ── Step 6: Auto cancel ───────────────────────────────────────────────
    async def do_cancel():
        return await cancel_order(order_id, merchant_id)

    cancelled_on_shopify = await step.run("auto-cancel-order", do_cancel)
    if not cancelled_on_shopify:
        return {"status": "cancel_failed", "order_id": order_id}
